...
def recompose(flat_state, row_length):
    for i in range(len(row_length)):
        flat_state.remove(i)
    S_T = np.asarray(flat_state).reshape(row_length)
    # S_T = np.ndarray([[flat_state[6], flat_state[7], flat_state[8], flat_state[9], flat_state[10], flat_state[11]],
