...
def get_w_reference(h_reference, r_reference, i_reference, theta_reference):
    return [0, -h_reference/r_reference, k_j2*np.sin(2*i_reference)*np.sin(theta_reference)]
...
def chen_jing_eom(t, state, rho_0, H, r_0, c_d, a_m_chaser, a_m_reference):
    ...
    dstate_dt[0][0] = mu / state[1]**2 - state[2]**2 / state[1]**3 + k_j2 / state[1]**4 * (1 - 3*np.sin(state[4])**2 * np.sin(state[5])**2)  # d v_z / dt
    dstate_dt[0][1] = - state[0] # d r / dt
    dstate_dt[0][2] = - k_j2 * np.sin(state[4])**2 * np.sin(2*state[5])**2 - .5 * rho * c_d a_m_reference * state[0] * v_reference # d h_reference / dt
    dstate_dt[0][3] = - 2 * k_j2 * np.cos(state[4])*np.sin(state[state[5]])  # d raan_reference / dt
    dstate_dt[0][4] = - k_j2 * np.sin(2*state[5])*npsin(2*state[4]) / 2 / state[2] / state[1]**3  # d i_reference / dt
    ...
    rho_chaser = get_rho(rho_0, r_chaser, r_0, H)
    ...
    dstate_dt[0][11] = w_bar * (state[8] - state[1]) + zeta*np.sin(state[5])*np.sin(state[4]) + state[] + f_drag_constant*(dstate_dt[0][8] + state[0] - state[6]*w_reference[1])  # d p3 / dt
    return dstate_dt


def j2_sedwick_propagator(state_0, rho_0, H, r_0, c_d, a_m_chaser, a_m_reference, time, step, type):
    sc = sp.integrate.ode(lambda t, x: chen_jing_eom(t, state, rho_0, H, r_0, c_d, a_m_chaser, a_m_reference)).set_integrator('dopri5', atol=1e-12,
                                                                                          rtol=1e-12)
