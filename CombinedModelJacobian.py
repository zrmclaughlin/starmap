...
def get_jacobian_second_test(c_d, a_m_reference, a_m_chaser, r_0, rho_0, H):
    wy = -h_reference / r_reference**2
    wz = k_j2 * sin(2*i_reference) * sin(theta_reference) / (h_reference * r_reference**3)

    r_chaser = Matrix([x, y, z - r_reference]).norm()  # r_chaser = np.linalg.norm([x, y, z - r_reference])
    z_chaser = x*cos(theta_reference)*sin(i_reference) - y*cos(i_reference) - (z - r_reference)*sin(i_reference)*sin(theta_reference)

    w_bar = -mu / r_chaser**3 - k_j2 / r_chaser**5 + 5*k_j2*z_chaser**2 / r_chaser**7
    zeta = 2*k_j2*z_chaser / r_chaser**5

    v_reference = Matrix([h_reference/r_reference, 0, v_z]).norm()  # v_reference = np.linalg.norm([h_reference/r_reference, 0, v_z])
    rho_reference = rho_0*exp(-(r_reference - r_0)/H)
    f_drag_reference = - .5*c_d*a_m_reference*rho_reference
    ...
    # v_chaser = [dx_dt - y*wz + (z - r_reference)*wy, dy_dt + x*wz, dz_dt + v_z - x*wy]
    v_chaser = Matrix([p1, p2, p3])
    rho_chaser = rho_0*exp(-(r_chaser - r_0)/H)
    f_drag_chaser = - .5*c_d*a_m_chaser*rho_chaser*v_chaser.norm()  # f_drag_chaser = - .5*c_d*a_m_chaser*rho_chaser*np.linalg.norm(v_chaser)
...
def main():
    ...
    # y = np_asarray([[1, 1, 1]])
    # print(x, y)
    # dstate_dt = np_concatenate((y, x), axis=0)
    # print(dstate_dt)
